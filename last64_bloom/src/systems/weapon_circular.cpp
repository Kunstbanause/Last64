...
namespace Actor {
    ...
    void WeaponCircular::fire(const T3DVec3& position, const T3DVec3& direction) {
        ...
        for (int i = 0; i < projectileCount + upgradeLevel + getProjectileCountBonus(); i++) {
            ...
            Projectile::spawn(spawnPos, fireDirection, getFinalProjectileSpeed(projectileSpeed), projectileSlowdown, projectileLifetime, getFinalDamage(), projectileColor, 1.3f);
        }
    }
