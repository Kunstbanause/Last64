...
namespace Actor {
    ...
    void WeaponCircular::fire(const T3DVec3& position, const T3DVec3& direction) {
        ...
        for (int i = 0; i < projectileCount + upgradeLevel + getProjectileCountBonus(); i++) {
            ...
            // Spawn projectile with circular direction, use player color instead of special color, slightly larger size
            uint32_t projectileColor = CIRCULAR_PROJECTILE_COLOR;
            if (player) {
                projectileColor = player->getColor();
            }
            Projectile::spawn(spawnPos, fireDirection, getFinalProjectileSpeed(projectileSpeed), projectileSlowdown, projectileLifetime, getFinalDamage(), projectileColor, 1.3f, getOwnerPlayerIndex(), weaponType);
        }
    }
